 ...
 *       [--day-sigma 0.3] [--max-per-day 4] [--seed 7]
 */
...
const base = { looks: SEQ_CONSTANTS.looks, rejectConfidence: SEQ_CONSTANTS.rejectConfidence, minProfitFactor: SEQ_CONSTANTS.minProfitFactor, bootstrap: { ...SEQ_CONSTANTS.bootstrap } };
const SCHEDULES: Schedule[] = [
    { name: 'A pre-registered', consts: { ...base, lookConfidences: SEQ_CONSTANTS.lookConfidences }, note: '99 / 97.5 / 96 / 95 (alphas sum 12.5%)' },
    { name: 'B Bonferroni-5%', consts: { ...base, lookConfidences: [0.9875, 0.9875, 0.9875, 0.9875] }, note: '4 × 1.25% = 5% bound' },
...
function simulateEpoch(rand: () => number, mean: number): RTrade[] {
    ...
    while (out.length < HORIZON) {
        ...
        const shock = normal(rand) * DAY_SIGMA;
...
function main(): void {
    console.log(`\n=== SEQUENTIAL-TEST CALIBRATION — ${SIMS} simulated epochs per cell, day shock σ ${DAY_SIGMA} R, 1..${MAX_PER_DAY} trades/day, seed ${SEED} ===`);
    console.log(`procedure: prefix looks at ${SEQ_CONSTANTS.looks.join('/')}; ACCEPT = LCB > 0 ∧ net R > 0 ∧ PF ≥ ${SEQ_CONSTANTS.minProfitFactor}; REJECT = UCB95 < 0; bootstrap ${SEQ_CONSTANTS.bootstrap.replicates} × seed ${SEQ_CONSTANTS.bootstrap.seed}, ≥ ${SEQ_CONSTANTS.bootstrap.minDays} days`);
    const t0 = Date.now();
