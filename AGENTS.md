...
- **Proposal lifecycle** (`src/services/trade-proposals.ts`): `open → executing → executed → closed` (or `rejected/expired/failed`). The LLM may create; only a human accept (or the paper-only auto-executor) places the bracket. The risk gate (`proposal-risk-gate.ts`) runs at creation AND acceptance; class caps are counted server-side from the DB so callers cannot understate the book.
