 ...
 *                          on a half-day; market calendar, review 2026-09-06)
 */
...
interface TradingJobDef {
    ...
    /** Earlier SEEDED schedules of this job: a stored job still carrying one
     *  of them was never tuned by the operator and follows the new seed
     *  (a tuned schedule is left alone). */
    legacySchedules?: CronSchedule[];
    message: string;
    model?: string;
...
/** Canonical JSON (sorted keys) so key order in the stored file cannot hide a match. */
...
/** Pure: does a stored job still carry one of the seed's legacy schedules? */
export function carriesLegacySchedule(job: Pick<CronJob, 'schedule'>, def: Pick<TradingJobDef, 'legacySchedules'>): boolean {
    const current = canonical(job.schedule);
    return (def.legacySchedules ?? []).some((s) => canonical(s) === current);
}
...
const TRADING_JOBS: TradingJobDef[] = [
    ...
    },
    {
        ...
        description: 'Pre-close review (30 min before the session close, half-day aware) and ranked overnight candidates (Opportunity Engine).',
        // 30 min before the CLOSE from the market calendar (15:30 ET; 12:30 ET on
        // a half-day). The fixed 15:30 cron never ran on a 13:00 close.
        schedule: { kind: 'session-close', offsetMin: 30, tz: 'America/New_York' },
        legacySchedules: [{ kind: 'cron', expr: '30 15 * * 1-5', tz: 'America/New_York' }],
        message: 'Run the pre-close review. First, evaluate open positions (ibkr_account) and their LIVE protection (ibkr_orders action list — open orders at the broker are the ONLY truth about brackets; the proposals store does not contain auto-protect GTC exits, so never call a position unprotected from proposal state alone): hold, trim, or close before the bell, applying the overnight limits from the risk rules (a DAY/intraday position is FLAT BY CLOSE: the 15:52 triage closes it unless the operator arms \'keep SYMBOL\' — recommend a keep with its reason, never assume one) — check EVERY position with earnings_calendar (action check, withinDays 2) and call out explicitly any position that would hold through a report (holding through a print is ONLY valid as an explicit earnings-bet; anything else must exit before the close) — and flag any position whose bracket exits are DAY orders about to expire (they leave the position unprotected overnight; suggest \'protect <SYMBOL> <stop> [target]\' to re-arm GTC exits). Second, the earnings-bet check (we are inside the entry window): call earnings_calendar (action day) for today\'s after-hours reporters and tomorrow\'s pre-market reporters; for the 2-3 most liquid interesting names, invoke the earnings-bet skill and run its evidence bar (earnings_bet_intel reactions — need >= 8 prints, >= 75% consistency on a side — plus at least one external signal: the reactions output\'s externalSignal (an open Polymarket beat market with real volume) counts, as do surprise streak, implied vs historical move, news, positioning). Where a name qualifies, register ONE labeled proposal (trade_proposals create, tradeClass "earnings-bet", tif "GTC", worstCaseGapPct from the reactions record, quantity OMITTED, expiresMinutes 25 so an unaccepted bet dies 5 minutes before the close, never surviving into the print) — the gate allows only one open bet; say plainly when no reporter qualifies, that is the normal outcome most days. Then call the opportunities tool with lane "overnight" (action "latest"; refresh if older than 15 minutes) — the overnight lane\'s OWN ranking (EOD continuation: significance in ATR units, closing strength vs VWAP, liquidity, RVOL; an excluded name carries its reason, and a lane score compares only inside this lane — the intraday composite is context, not the ranking) — and assess its top candidates for overnight holds — and report in ONE line any watchlist symbol that topped recent scan cycles just under the trigger threshold (e.g. score 75-79): visibility, not action; a +10% day on a watchlist name must never pass in silence. For the candidates: check earnings_calendar and news catalysts (web_search), prefer lower-ATR names, and validate each with risk_manager using the overnight position limits. Also call event_risk (action macro, withinDays 1): on a macro binary night (high-uncertainty CPI/FOMC/jobs print tomorrow), a qualifying overnight setup must name the event in its rationale — prefer passing on marginal ones. Register at most 2 qualifying overnight setups with trade_proposals (action create, strategyId "overnight", setupId "eod-continuation" (or the setup you see), tif "GTC" so the exits survive the close, expiresMinutes so the ENTRY dies by the bell (the review starts 30 min before the close → 25; the contract refuses an expiry past the close — an unfilled overnight entry never arms tomorrow), quantity OMITTED so the overnight budget and the gap-stress cap size it; the position exits at 10:00 ET tomorrow at the latest) and present each with direction, entry, stop, target, size, rationale, and the proposal ID with "Reply \'accept <ID>\' to execute (paper)". Be explicit when nothing qualifies. Do not place orders yourself.',
        model: undefined,
        maxIterations: 24,
        // Wide enough for a 12:30 half-day fire; the executor's session-close
        // guard refuses a run past the day's close whatever the window says.
        activeStart: '11:30',
        activeEnd: '16:00',
    },
];
...
export function ensureTradingCronJobs(): void {
    ...
    for (const def of TRADING_JOBS) {
        ...
        if (existing) {
            ...
            // (user-tunable) — EXCEPT a job still carrying a legacy SEED
            // schedule, which follows the new seed (review 2026-09-06: the
            // Pre-Close Review moved from a fixed 15:30 to close-relative).
            if (carriesLegacySchedule(existing, def) && JSON.stringify(existing.schedule) !== JSON.stringify(def.schedule)) {
                existing.schedule = def.schedule;
                existing.activeHours = { start: def.activeStart, end: def.activeEnd, timezone: 'America/New_York', daysOfWeek: WEEKDAYS };
                existing.state.nextRunAtMs = computeNextRunAtMs(def.schedule, now);
