...
function handleManualExitFill(orderId: number, avgFillPrice: number): void {
    ...
    if (attachedApi) {
        ...
        void (async () => {
            // Round-12 review: the delayed fill ALWAYS verifies the
            // position — a resting close can fill an outdated quantity
            // and leave residue with an otherwise clean cleanup, so
            // gating the check on a cleanup incident missed exactly that
            // case. A thrown cleanup notifies too, marked unverified.
            try {
                const { cleanupExitsAfterClose, fetchPositions } = await import('./position-actions.js');
                let cleanupClean = false;
                let cleanupNote = 'cleanup FAILED (threw) — nothing about the exit book is verified';
                try {
                    ...
                    cleanupClean = r.verified && !r.exitFilledDuringClose && r.stillWorking.length === 0;
                    cleanupNote = `cleanup verified=${r.verified}${r.exitFilledDuringClose ? ', an exit ALSO filled' : ''}` +
                        `${r.stillWorking.length > 0 ? `, ${r.stillWorking.length} order(s) STILL WORKING` : ''}`;
                } catch (err) {
                ...
                try {
                    ...
                    if (after && after.quantity !== 0) {
                        posNote = `position NOT FLAT: ${after.quantity > 0 ? 'LONG' : 'SHORT'} ${Math.abs(after.quantity)} remains`;
                    } else {
                        confirmedFlat = true;
                        posNote = 'position book confirms flat';
                    }
                } catch { /* posNote already says unverified */ }
                if (!confirmedFlat || !cleanupClean) {
                    logger.error(`[outcome-tracker] ${manual.symbol}: delayed-fill settlement incomplete (${cleanupNote}; ${posNote})`);
                    await notifyAutoProtect(
                        `⚠️ ${manual.symbol}: the resting close FILLED, but settlement is not fully confirmed — ` +
                        `${cleanupNote}; ${posNote}. Review 'positions'/'orders'; a resting exit on a flat ` +
                        `symbol OPENS a position when it fills.`,
                    ).catch(() => { /* alert bridge down — the error log above stands */ });
                }
            } catch (err) {
                logger.error(`[outcome-tracker] delayed-fill settlement check ${manual.symbol} failed entirely: ${err}`);
            }
        })();
    }
