...
describe('laneRankFor (REQ-DISC-003 — server-side provenance)', () => {
    const snapshot = { opportunities: [{ symbol: 'MU', compositeRank: 78 }], lanes: buildSnapshotLanes([opp()]) };
    const patternScan = { candidates: [{ symbol: 'CUP1', matches: [{ pattern: 'flat-base', score: 80 }, { pattern: 'cup-and-handle', score: 72 }] }] };
    test('intraday: the trigger rank, else the composite; overnight: the lane score; cup: the detector score; swing/bet: none', () => {
        expect(laneRankFor('intraday', 'MU', { triggerRank: 66, snapshot, patternScan })).toEqual({ laneRank: 66, rankerVersion: 'composite-v1' });
        expect(laneRankFor('intraday', 'mu', { triggerRank: null, snapshot, patternScan })).toEqual({ laneRank: 78, rankerVersion: 'composite-v1' });
        expect(laneRankFor('overnight', 'MU', { triggerRank: null, snapshot, patternScan })).toEqual({ laneRank: 70, rankerVersion: 'eod-continuation-v1' });
        expect(laneRankFor('cup-and-handle', 'CUP1', { triggerRank: null, snapshot, patternScan })).toEqual({ laneRank: 72, rankerVersion: 'detector-v1' });
        expect(laneRankFor('swing', 'MU', { triggerRank: 66, snapshot, patternScan })).toEqual({ laneRank: null, rankerVersion: null });
        expect(laneRankFor('earnings-bet', 'MU', { triggerRank: null, snapshot, patternScan })).toEqual({ laneRank: null, rankerVersion: null });
    });

    test('a symbol the ranker never saw: null rank, the version recorded', () => {
        expect(laneRankFor('overnight', 'XYZ', { triggerRank: null, snapshot, patternScan })).toEqual({ laneRank: null, rankerVersion: 'eod-continuation-v1' });
        expect(laneRankFor('overnight', 'MU', { triggerRank: null, snapshot: null, patternScan: null })).toEqual({ laneRank: null, rankerVersion: 'eod-continuation-v1' });
        expect(laneRankFor('intraday', 'XYZ', { triggerRank: null, snapshot, patternScan })).toEqual({ laneRank: null, rankerVersion: 'composite-v1' });
    });
});
