 ...
 *               each; one-sided LCB confidence 99 / 97.5 / 96 / 95 %
 *               (O'Brien-Fleming-style spending — an early ACCEPT needs a
 *               big effect; the familywise false-ACCEPT rate stays ≈5 %)
 ...
 *               and no active stop (queued for the operator); automatic
 *               step-down at −5 % from the last step-up mark
...
function byDay(trades: RTrade[]): Map<string, number[]> {
...
function profitFactor(trades: RTrade[]): number {
    let wins = 0, losses = 0;
    for (const t of trades) {
...
/** REQ-SEQ-002/003: evaluate one look boundary over the epoch's trades. */
export function evaluateLook(trades: RTrade[], lookN: number): LookResult {
    const idx = SEQ_CONSTANTS.looks.indexOf(lookN as 25 | 50 | 75 | 100);
    const lookConfidence = idx >= 0 ? SEQ_CONSTANTS.lookConfidences[idx] : SEQ_CONSTANTS.rejectConfidence;
    ...
    const upNeg = dayBlockBootstrapLcb(negated, { replicates: b.replicates, seed: b.seed, minDays: b.minDays, alpha: 1 - SEQ_CONSTANTS.rejectConfidence });
    ...
    if (lcb > 0 && sumR > 0 && pf >= SEQ_CONSTANTS.minProfitFactor) {
        decision = 'ACCEPT';
        reasons.push(`LCB ${lcb.toFixed(3)} > 0 at ${lookConfidence * 100}%`, `net R ${sumR.toFixed(2)} > 0`, `PF ${pf.toFixed(2)} ≥ ${SEQ_CONSTANTS.minProfitFactor}`);
    } else if (ucb95 < 0) {
        decision = 'REJECT';
        ...
        if (pf < SEQ_CONSTANTS.minProfitFactor) reasons.push(`PF ${pf.toFixed(2)} < ${SEQ_CONSTANTS.minProfitFactor}`);
...
export function runningStats(trades: RTrade[]): RunningStats {
    const n = trades.length;
    const sumR = trades.reduce((s, t) => s + t.netR, 0);
    return {
        n,
...
export function ladderEligibility(input: { n: number; sumR: number; rung: number; stopActive: boolean }): LadderEligibility {
